...
bool board_check_valid (const board_t * board)
{
   int colour;
   const sq_t * ptr;
   int sq, piece;
...
void board_save_to_str(board_t * board, char *s)
{
  strcpy(s,"White:");
  for (int i=0; i<64; i++) {
    if (i%8 == 0)
      strcat(s,"\n");
    int sq = Y_COORD(i)*8 + (7-X_COORD(i));
    if (board->square[SQUARE_FROM_64(sq)] & WhiteFlag)
      strcat(s,"*");
    else
      strcat(s,"_");
  }
  ...
  for (int i=0; i<64; i++) {
    if (i%8 == 0)
      strcat(s,"\n");
    if (board->square[SQUARE_FROM_64(i)] & BlackFlag)
      strcat(s,"*");
    else
      strcat(s,"_");
  }
